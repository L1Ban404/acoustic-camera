...
本工程采用 Zynq-7020 声学相机架构。项目设计说明覆盖两种不同的配置，用于解释架构选择与性能口径：

| 资料 | 描述的配置 | 在本文档中的用途 |
| --- | --- | --- |
| 《基于 Zynq 的声学相机》PDF | 8 麦克风、128×72 热力图、60 FPS 级可视化 | 模块职责、PS/PL 分工、视频叠加与上位机控制 |
| 《高帧率低功耗的声学相机系统》DOCX | 16 通道、分数延迟、滑窗累加、Zynq-7020 | 算法架构、采样/缓存策略和论文评测口径 |

因此，`8` 与 `16` 通道、`60 FPS` 与“每秒约两万次定位/网格更新”等数值不可直接并列比较。仓库中的 Block Design 和 RTL/XCI 是工程实现的代码真源；文档数值只在对应配置下成立。
...
*图：16 麦克风双圆环阵列。标注尺寸属于该实验配置，实际硬件应重新核对。*

## 资源与评测结果（设计报告值）

16 通道配置报告的 Zynq-7020 资源为 LUT 17,543（32.81%）、FF 29,080（27.33%）、BRAM 119（85%）和 DSP 97（44.09%）。BRAM 是最紧张资源，应在修改缓存深度、视频缓存或时延表宽度前先评估。
...
这些设计说明用于架构与性能口径说明；工程的构建、综合和版本控制不依赖它们。
